...
class DataframeInitiator:
    ...
    def _add_time_series_features(self, df):
        ...
        # ЛАГИ - используют только прошлые значения (корректно)
        for lag in [1, 2, 4, 8]:
            df[f'lag_{lag}'] = df.groupby(group_cols)[target_col].shift(lag)

        # СКОЛЬЗЯЩИЕ СРЕДНИЕ - используют ТОЛЬКО прошлые значения
        # Ключевой момент: shift(1) гарантирует, что не используем текущее значение
        for window in [4, 8, 12]:
            df[f'roll_mean_{window}'] = df.groupby(group_cols)[target_col].transform(
                lambda x: x.shift(1).rolling(window, min_periods=1).mean()
            )

        # Логарифмические лаги - для лучшего масштабирования
        # Сначала создаем логарифмированные продажи
        df['log_sales'] = np.log1p(df[target_col])

        for lag in [1, 2, 4, 8]:
            df[f'log_lag_{lag}'] = df.groupby(group_cols)['log_sales'].shift(lag)

        # Скользящие средние от логарифмов
        for window in [4, 8, 12]:
        ...
        # Заполняем NaN (первые недели, где нет истории)
        df.fillna(0, inplace=True)

        return df

    def _add_base_features(self):
        """Базовые признаки без лагов"""
        # 1. Базовый датафрейм продаж
        ...
        # 8. Сортировка и индекс
        df_weekly = df_weekly.sort_values(['Контрагент Код', 'Номенклатура Код', 'Year_Week'])
        df_weekly['Week_Index'] = df_weekly.groupby(['Контрагент Код', 'Номенклатура Код']).cumcount()

        return df_weekly

    def _add_time_series_features_proper(self, df):
        """
        ПРАВИЛЬНОЕ добавление временных признаков
        ВАЖНО: этот метод должен вызываться ДО разбиения на train/test,
        но при обучении модель будет использовать эти признаки корректно,
        так как они основаны ТОЛЬКО на прошлых значениях
        """
        # Сортировка обязательна
        df = df.sort_values(['Контрагент Код', 'Номенклатура Код', 'Year_Week'])
